...
def process_ocr_pdf_with_tesseract(pdf_path):
    ...
    try:
        ...
        result = subprocess.run([
            './tesseract_pdf_ocr.sh', str(pdf_path)
...
def main():
    ...
    for pdf_file in to_process:
        ...
        if not result:
            ...
            # TODO: Zmodyfikuj tesseract_pdf_ocr.sh żeby obsługiwał różne pliki
    ...
    if processed > 0:
        print(f"\n🔄 Uruchom ponownie corpus_builder.py żeby zaktualizować korpus!")
